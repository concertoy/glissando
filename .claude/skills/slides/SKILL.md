...
- For diagrams: prefer `/figure-diagram` (built-in `diagramBox` + `arrow` + `connector` on `blank()` slides). If too complex, use `/figure-figma` (Figma MCP) or `/figure` (AI generation). See `CLAUDE.md` for the full component API.
